...
namespace RonftonCard.Core.CardReader
{
	...
	public abstract class AbstractCardReader : ICardReader
	{
		...
		public abstract bool ReadBlock(int blockNum, out byte[] data, int len);
		public abstract bool ReadSector(int sectorNum, out byte[] data, int len);

		public abstract bool WriteBlock(int blockNum, byte[] data, int len);
		#endregion
