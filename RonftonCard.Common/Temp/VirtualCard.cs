...
namespace RonftonCard.Common
{
	public class VirtualCard
	{
		...
		{
			Array.Copy(data, 0, this.virtualAddress, offset,  length);
		}

		public void WriteEntity<ET>(ET entity)
		{
			if (entity == null)
				return;
			...
			{
				CardDataDescriptor descriptor = GetItemDescriptor(p.Name);
				if(descriptor != null)
				{
					byte[] b = ArrayHelper.CopyFrom(DataTypeHandler.ToByte(p.GetValue(entity), descriptor.DataType), descriptor.Length);
					if (b != null)
					{
						WriteCard(descriptor.Offset, b, descriptor.Length);
					}
				}
			}
		}

		private CardDataDescriptor GetItemDescriptor(String name)
		{
			//return cardContext.CardTemplete.DataDescriptor.FirstOrDefault(item => item.Name.Equals(name, StringComparison.CurrentCultureIgnoreCase));
			return null;
		}
